...
class MessageQueueService {
    ...
    messageCountPerMinute = new Map();
    messageCountPerHour = new Map();
    recipientMessageCount = new Map();
    constructor() {
    ...
    cleanupOldTimestamps() {
        ...
        for (const [session, timestamps] of this.messageCountPerMinute.entries()) {
            this.messageCountPerMinute.set(session, timestamps.filter((t) => t > oneMinuteAgo));
        }
        // Cleanup per-hour counters
        for (const [session, timestamps] of this.messageCountPerHour.entries()) {
            this.messageCountPerHour.set(session, timestamps.filter((t) => t > oneHourAgo));
        }
        // Cleanup per-recipient counters
        for (const [__session, recipients] of this.recipientMessageCount.entries()) {
    ...
    checkRateLimit(sessionId, to) {
        ...
        const messagesToRecipient = recipientTimestamps.filter((t) => t > oneHourAgo).length;
    ...
    updateStats(sessionId, status, delta) {
