...
export class SessionManagementService {
     ...
     */
    static async validateSessionCreation(sessionId, phoneNumber) {
        try {
    ...
    static async createSession(options) {
        ...
        try {
            ...
            const account = await WhatsappAccountService.createAccount(userId, sessionId, normalizedPhone);
     ...
     */
    static async onSessionConnected(sessionId, connectionInfo) {
        try {
            ...
            if (connectionInfo?.user?.id) {
                ...
                if (phoneNumber) {
                    ...
                    if (existingSession && existingSession.sessionId !== sessionId) {
                        ...
                        throw new Error(`Phone number ${PhoneService.format(phoneNumber)} is already connected to another session`);
                    }
     ...
     */
    static async onSessionDisconnected(sessionId) {
        try {
     ...
     */
    static async onSessionConnecting(sessionId) {
        try {
            // Check if account exists
            const account = await WhatsappAccountService.findBySessionId(sessionId);
            if (!account) {
     ...
     */
    static async cleanupInactiveSessions(inactiveHours = 24) {
        try {
