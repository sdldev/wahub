...
export class MessageHistoryService {
     ...
     */
    static async saveMessage(sessionId, from, to, content, type = 'text', status = 'pending') {
        try {
     ...
     */
    static async updateStatus(messageId, status, error) {
        try {
            const result = await db
                .update(messages)
                .set({
     ...
     */
    static async incrementRetryCount(messageId) {
        try {
     ...
     */
    static async getBySession(sessionId, limit = 100, offset = 0) {
        try {
            return await db
                .select()
                .from(messages)
     ...
     */
    static async getByDateRange(sessionId, startDate, endDate) {
        try {
            return await db
                .select()
                .from(messages)
                .where(and(eq(messages.sessionId, sessionId), gte(messages.createdAt, new Date(startDate)), lte(messages.createdAt, new Date(endDate))))
                .orderBy(desc(messages.createdAt));
        }
        catch (error) {
     ...
     * Get failed messages
     */
    static async getFailedMessages(sessionId) {
        try {
            return await db
                .select()
                .from(messages)
                .where(and(eq(messages.sessionId, sessionId), eq(messages.status, 'failed')))
                .orderBy(desc(messages.createdAt));
        }
        catch (error) {
     ...
     */
    static async getStatistics(sessionId, startDate, endDate) {
        try {
            let allMessages;
